...
## 前置:确认 hr-cli 可调用(每次会话开始时 MUST 先做)

Agent 子进程常是非交互 shell,**不读 `~/.zshrc`**,可能导致 `which hr-cli` 返回空,即使二进制已通过 `go install` 装好。
...
- `which` 成功 → 直接用 `hr-cli ...`
- `which` 失败但 `ls` 成功 → PATH 缺失,用绝对路径 `~/go/bin/hr-cli ...` 调用即可(不要让用户重装)
- 两者都失败 → 引导用户执行 `go install github.com/TsCarpe/hr-cli@latest`
...
**查不到的术语**:先用 `hr-cli which <关键词>` 模糊搜(支持同义词,如 `which 邀课` / `which 查教师`),它同时搜 shortcut 和 service.method 的描述。仍查不到再用 `hr-cli schema` 列全部。**本表仍是单一真相源**,which 只是辅助发现,不维护两份映射。
...
```
推荐: hr-cli doctor           # 一次自检 配置/token/连通性/schoolId,FAIL → exit 1
       hr-cli --agent doctor  # AI 消费:输出结构化 JSON
旧法: 看配置文件 ~/.hr-cli/config.json 是否存在且 hrToken 非空
       直接调任意 read 接口,401 = 未登录
```
...
`saas +login` 成功后,hrToken + 用户身份 + 当前学校上下文都写入 `~/.hr-cli/config.json`。后续操作需要的上下文**从这里取,不要反问用户**。

**上下文字段**(config.json 顶层 = 当前学校上下文,`userInfo` = 用户身份):
- `userInfo.staffId` — 你的教职工 ID(write 操作的"授课老师"参数,如建课/邀课)
- `userInfo.accountName` — 你的姓名(跟用户对话时用)
- `userInfo.userId` — saas 应用层 ID(跨学校不变)
- `schoolId` / `campusId` / `tenantId` — 当前学校/校区/租户(**切学校会变**)
- `schoolName` / `campusName` — 展示用

**怎么拿(两条路)**:
- 推荐:`hr-cli --agent doctor` 一次调用,JSON 输出的 `context` 字段含上述全部
- 备选:直接 Read `~/.hr-cli/config.json`(诊断或 doctor 不可用时)

**关键约束(默认不反问用户)**:
1. **接口要 schoolId/campusId 时不要问用户** —— 框架已在 body 层自动注入(`internal/runner/runner.go` RunMethod),用户传的值优先,缺失就从 config.json 兜底
2. **staffId 跟学校走** —— 切换学校后 staffId 会变,不要跨学校复用旧值;拿不准就重新读 config.json

### 第一人称指代解析(所有 write 场景通用)

用户开口常带"我/我自己/给我/帮我",这些**第一人称代词 = 当前登录用户**。解析规则:

**关键**:出现"我/自己/给我/帮我"时,**MUST 先把当前用户代入对应角色**,不要默认把后续出现的名字塞进主角色。例:
- ✅ "给我建一节课,邀请许晗艺" → 授课老师=我(`userInfo.staffId`),邀请评课人=许晗艺
- ❌ 错误解读:把许晗艺当授课老师(忽略了"给我"锁定的指代)

**判断指代优先级**(从高到低):
1. 明确人名 → 该名字对应的角色(看动词和语境)
2. 第一人称代词 → 当前登录用户(`userInfo.staffId`)
3. 缺失 → 该角色空缺,按场景处理(反问用户 or 用默认值)

**为什么这条规则在 hr-shared**:指代解析不是邀课专属 —— 评课、查课、待办等所有 write 场景都会遇到"给我/帮我/我",各领域 skill 都假定你已经会处理。
